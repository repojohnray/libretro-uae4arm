...
static bool getsem1 (int unitnum)
{
...
static int get_standard_cd_unit2 (enum cd_standard_unit csu)
{
	int unitnum = 0;
	int isaudio = 0;
...
static int do_scsi5 (int unitnum, uae_u8 *cmd, int cmdlen, uae_u8 *out, int outsize)
{
...
/* play CD audio */
int sys_command_cd_play4 (int unitnum, int startlsn, int endlsn, int scan)
{
	int v;
	if (failunit (unitnum))
		return 0;
...
int sys_command_cd_play (int unitnum, int startlsn, int endlsn, int scan, play_status_callback statusfunc, play_subchannel_callback subfunc)
{
	...
	if (!getsem1 (unitnum))
		return 0;
	if (device_func[unitnum]->play == NULL)
		v = sys_command_cd_play4 (unitnum, startlsn, endlsn, scan);
	else
...
int sys_command_cd_qcode (int unitnum, uae_u8 *buf)
{
	...
	if (device_func[unitnum]->qcode == NULL) {
		...
		v = do_scsi5 (unitnum, cmd, sizeof cmd, buf, SUBQ_SIZE);
	} else {
...
int sys_command_cd_toc (int unitnum, struct cd_toc_head *toc)
{
	...
	if (device_func[unitnum]->toc == NULL) {
		...
		if (do_scsi5 (unitnum, cmd, sizeof cmd, buf, size)) {
			// toc parse to do
			v = 0;
		}
		v = 0;
...
int sys_command_cd_read (int unitnum, uae_u8 *data, int block, int size)
{
	...
	if (device_func[unitnum]->read == NULL) {
		...
		v = do_scsi5 (unitnum, cmd1, sizeof cmd1, data, size * 2048);
#if 0
		if (!v) {
			uae_u8 cmd2[12] = { 0xbe, 0, block >> 24, block >> 16, block >> 8, block >> 0, size >> 16, size >> 8, size >> 0, 0x10, 0, 0 };
			v = do_scsi5 (unitnum, cmd2, sizeof cmd2, data, size * 2048);
		}
#endif
	} else {
...
int sys_command_cd_rawread5 (int unitnum, uae_u8 *data, int block, int size, int sectorsize)
{
	int v;
	if (failunit (unitnum))
		return -1;
	...
	if (device_func[unitnum]->rawread == NULL) {
		...
		v = do_scsi5 (unitnum, cmd, sizeof cmd, data, size * sectorsize);
	} else {
...
int sys_command_cd_rawread8 (int unitnum, uae_u8 *data, int block, int size, int sectorsize, uae_u8 sectortype, uae_u8 scsicmd9, uae_u8 subs)
{
	int v;
	if (failunit (unitnum))
		return -1;
...
int sys_command_read (int unitnum, uae_u8 *data, int block, int size)
{
	...
	if (device_func[unitnum]->read == NULL) {
		...
		v = do_scsi5 (unitnum, cmd, sizeof cmd, data, size * 2048);
	} else {
		v = device_func[unitnum]->read (unitnum, data, block, size);
	}
	freesem (unitnum);
...
static int scsi_read_cd (int unitnum, uae_u8 *cmd, uae_u8 *data, struct device_info *di)
{
	...
	if (len == 0)
		return 0;
	return sys_command_cd_rawread8 (unitnum, data, start, len, 0, (cmd[1] >> 2) & 7, cmd[9], subs);
}
	...
	*reply_len = *sense_len = 0;
	...
	case 0x43: // READ TOC
		...
		case 0xba: // SCAN
		{
			...
			sys_command_cd_play4 (unitnum, start, end, scan);
			scsi_len = 0;
		}
		break;
		case 0x48: // PLAY AUDIO TRACK/INDEX
		{
			...
			sys_command_cd_pause (unitnum, 0);
			if (!sys_command_cd_play4 (unitnum, start, end, 0))
				goto notdatatrack;
			scsi_len = 0;
		}
		break;
		...
		case 0xa9: // PLAY AUDIO TRACK RELATIVE (12)
		{
			...
			if (len > 0) {
				sys_command_cd_pause (unitnum, 0);
				if (!sys_command_cd_play4 (unitnum, start, start + len, 0))
					goto notdatatrack;
			}
			scsi_len = 0;
		}
